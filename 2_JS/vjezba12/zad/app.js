...
function izracunajStatistiku() {
  ...
  const sumaBodova = filtriraniPodaci.reduce(
    (ukupno, k) => ukupno + k.bodovi,
    0,
  );
  ...
  const pozicijaPolaznik = sviKorisnici.findIndex(
    (k) => k.ime === "Mate" && k.prezime === "Rimac",
  );
  //   ispis.innerHTML = `
...
function kreirajCelijeDOM(red, korisnik) {
  ...
  poljeBodovi.textContent = korisnik.bodovi.toLocaleString("hr-HR", {
